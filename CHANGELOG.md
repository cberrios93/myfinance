...
- **Técnico** — Nuevo endpoint `api/cotizar-viaje.ts` (Edge Function, Haiku 4.5). Passthrough idéntico a `api/boleta.ts` — el frontend construye el payload Anthropic completo (`model`, `system`, `messages`). Proxy Vite añadido en `vite.config.ts` para dev local.
- **Fix** — Cotización IA: el frontend ahora envía payload Anthropic completo en lugar de parámetros de app. Strip de markdown fences (` ```json `) en la respuesta de Haiku antes de parsear. `useMemo` ilegal dentro de JSX convertido a JSX directo.

#### Módulo Eventos de Vida — Mejoras (Puntos 1, 2 y 3)
- **Mejora** — **Punto 1: Selector mes+año en todos los eventos.** Todos los tipos de evento de vida permiten ahora seleccionar mes y año de inicio (no solo año). El mes se persiste en campos opcionales `mes` (RetiroUnico) y `mesInicio`/`mesFin` (GastoRecurrente) en `types.ts`. La visualización en el resumen muestra "mar 2030" en lugar de solo "2030". Fix incluido: el selector año/mes que antes estaba oculto para eventos con calculadora de préstamo (vivienda, auto) ya aparece correctamente.
- **Mejora** — **Punto 2: Modo detallado por etapa en Nacimiento de Hijo/a.** Cada etapa (Bebé, Nido, Colegio, Universidad, Post-uni) tiene un toggle "Detallar gastos →" que activa modo detallado con 4 campos: Pensión mensual (S//mes), Matrícula anual (S//año, mensualizada automáticamente ÷12), Médico/salud (S//mes), Alimentación/ropa/ocio (S//mes). El total mensual se calcula en tiempo real. En modo básico sigue funcionando con un solo campo S//mes. El motor de proyección recibe el valor mensualizado consolidado en ambos modos. Selector mes+año de nacimiento añadido.
- **Feature** — **Punto 3: Nuevo tipo de evento "Alquiler".** Wizard dedicado para modelar períodos de alquiler: nombre, moneda (PEN/USD con TC Rextie), monto mensual, mes+año inicio, mes+año fin. Muestra duración en meses y estimado total. Genera un `gastoRecurrente` con `tipoEvento: 'alquiler'` (color cian). Los wizards de Posgrado y Matrimonio también actualizados para usar mes+año en sus fechas.
